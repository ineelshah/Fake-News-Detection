...
# compute correlation between fake fraction and trend fraction 
def computeCorrelation(X,Y):
	correlation_df = pd.DataFrame()
	correlation_df['fakeFraction']=X
	correlation_df['trendFraction']=Y
	return correlation_df['fakeFraction'].corr(correlation_df['trendFraction'])

# compute correlations for all categories and all window sizes
def computeCorrelationMatrix(input_df):
	minDate,maxDate=minmaxPublishedDate(input_df)
	...
	for i in ['business', 'politics', 'sport', 'entertainment', 'tech']:
		for j in [3,5,7,9]:
			...
			for point in points:
				X.append(point[0])
				Y.append(point[1])
				correlationMatrix[i][j] = computeCorrelation(X,Y)
